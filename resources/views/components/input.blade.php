<div>
    ...
    <div @class(['mt-2', 'relative rounded-md shadow-sm'=> $errors->has($name) && $type !== 'file'])>
        <input id="{{ $id }}" name="{{ $name }}" type="{{ $type }}" @if($type!=='file' ) value="{{ old($name) ?? $value }}" @class([ 'form-input block w-full rounded-md border-0 py-1.5 ring-1 ring-inset focus:ring-2 focus:ring-inset sm:text-sm sm:leading-6' , 'pr-10 text-red-900 ring-red-300 placeholder:text-red-300 focus:ring-red-500'=> $errors->has($name),
